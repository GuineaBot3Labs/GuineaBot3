    ...
    if time_difference - time.time() < 90:
        ...
        try:
            ...
            if games == None:
                ...
                time_control_minutes = random.randint(17, 40)  # Randomly choose a time control between 6 to 40 minutes
                # time_control_minutes = random.randint(13, 45)
                print(f"minutes: {time_control_minutes}")
                time_control = time_control_minutes * 60  # Convert to seconds


                try:
                    print(f"Challenging {bot_to_play['id']} with time {time_control_minutes} for increment of minutes")
                    client.challenges.create(bot_to_play['id'], variant='standard', clock_limit=time_control, clock_increment=0, rated=False)
